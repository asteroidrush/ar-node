...
try:
    ...
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.eosioAccount, stakedDeposit=500000, waitForTransBlock=False, exitOnError=True)
    trans=nodes[0].setAccountRam(contractAccount, 100)
    ...
    Print("Set contract host to %s" % (contractAccount.name))
    trans=nodes[0].setContractHost(contractAccount.name, True)
    if trans is None:
        Utils.cmdError("ERROR: Failed to set contract host to %s." % (contractAccount.name))
