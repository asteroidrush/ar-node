...
class Node(object):
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False, header_state=False):
        """Given a blockId will return block details."""
        assert(isinstance(blockNum, int))
        if not self.enableMongo:
            cmdDesc="get block" + ['', ' --header-state'][header_state]
    ...
    def setContractHost(self, account, waitForTransBlock=False,  shouldFail=False):
        cmd = "%s %s set account contracthost -j %s 1" % (Utils.EosClientPath, self.eosClientArgs(), account)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
    ...
    def setAccountPermission(self, account, permission, keys, accounts):
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        try:
            trans=self.processCleosCmd(cmd, "Set account permission %s " % permission, returnType=ReturnType.json, silentErrors=False)
        except subprocess.CalledProcessError as ex:
            msg=ex.output.decode("utf-8")
    ...
    def getSchedule(self):
        ...
        cmd="%s" % (cmdDesc)
        msg="(get producer schedule)"
        return self.processCleosCmd(cmd, cmdDesc, silentErrors=False, exitOnError=False, exitMsg=msg, returnType=ReturnType.raw)

    def getProducers(self):
        cmdDesc="system listproducers"
        cmd="%s" % (cmdDesc)
        msg="(get producer list)"
        return self.processCleosCmd(cmd, cmdDesc, silentErrors=False, exitOnError=False, exitMsg=msg, returnType=ReturnType.raw)
    ...
    def getNextCleanProductionCycle(self, trans):
        ...
        # Wait for setup of pendin
        self.waitForIrreversibleBlock(promotedBlockNum, timeout=rounds/2)

        # Wait until pending schedule will be applied
        self.waitForIrreversibleBlock(self.getHeadBlockNum(), timeout=rounds/2 )
